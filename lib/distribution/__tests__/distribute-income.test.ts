import {
  getExtraDebtPaymentEntries,
  getExtraDebtPaymentTotal,
} from '@/lib/allocation-extra';
import {
...
function totalAllocated(alloc: ReturnType<typeof distributeIncome>): number {
  const { needs, minimumPayments } = alloc;
  let total = 0;
...
describe('distributeIncome', () => {
  ...
  it('should return empty allocation when income is 0', () => {
    ...
    expect(result.extraDebtPayments).toBeUndefined();
  ...
  it('should cover all tiers and apply snowball when income is sufficient', () => {
    ...
    expect(getExtraDebtPaymentEntries(result)[0]).toEqual({
      debtId: 'd1',
      amount: 400,
    });
  ...
  // ─── Fixed Tier 1 priority ─────────────────────────────────────────
  it('should use food before housing in Tier 1 waterfall', () => {
    const state = makeState({
    ...
    const result = distributeIncome(1200, state, testDate);

    expect(result.needs.food).toBe(600);
    expect(result.needs.housing).toBe(600);
  });
  ...
  it('should break snowball ties alphabetically by label', () => {
    ...
    expect(getExtraDebtPaymentEntries(result)[0]?.debtId).toBe('d2'); // Alpha
  });
  ...
  it('should ignore closed debts in minimum payments and snowball', () => {
    ...
    expect(getExtraDebtPaymentEntries(result)[0]?.debtId).toBe('d2');
  });
